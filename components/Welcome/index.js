...
const Welcome = (props) => {
    ...
    const { connectWallet, isWalletConnected, isPaused, isWhitelistMintEnabled, isUserInWhitelist, isSoldOut, generateMarketplaceUrl } = props;
    ...
    const renderPurchase = () => {
        if(soldOut) {
            return (
            <div>
                <h2>Tokens have been <strong>sold out</strong>! <span>🥳</span></h2>

                You can buy from our beloved holders on <a href={generateMarketplaceUrl()} target="_blank">{CollectionConfig.marketplaceConfig.name}</a>.
            </div>
        );
        }
        if(canMint()) {
            return (<div className={styles.btn} onClick={handleMintPage} >MINT NOW</div>);
        }
        return null;
    }
...
"Welcome to the community! I am happy to announce the launch of Lamar's Odom's Metaverse! It will be featuring my life as a professional athlete as well as my road to reborn in the form of digital arts inspired by the NFT movement. This will be a ongoing project that evolves over time and I want to create a loving and supporting community. One love!" - Lamar Odom
                ...
                { walletConnected ?
                    null
                    : <div className={styles.btn} onClick={handleConnectWallet} >Coming soon</div>
                }
                { renderPurchase() }
            </div>
        </div>
